...
# Load a pre-trained DenseNet model for feature extraction
startTime = time.time()  # Record start time for loading DenseNet model
print("Loading pre-trained DenseNet model...")  # Print progress message
model = models.densenet201(pretrained=True)  # Load pre-trained DenseNet model
model.eval()  # Set model to evaluation mode (not training mode)
featureExtractor = torch.nn.Sequential(
    *list(model.children())[:-1]
)  # Extract feature extractor from DenseNet model
endTime = time.time()  # Record end time for loading DenseNet model
print(
    f"Loaded DenseNet model in {(endTime - startTime):.2f} seconds."
)  # Print loading time
